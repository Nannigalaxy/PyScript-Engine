...
class CustomDropdownMenu(customtkinter.CTkFrame):
        ...
        **kwargs,
        ...
        if widget.master.winfo_name().startswith("!ctktitlemenu"):
            ...
            widget.winfo_toplevel().bind(
                "<ButtonPress>", self._checkIfMouseLeft, add="+"
            )
    ...
    def add_submenu(self, submenu_name: str, **kwargs) -> "CustomDropdownMenu":
        ...
        submenuButtonSeed.bind(
            "<Enter>", lambda e: self.after(500, lambda: submenu._show_submenu(self))
        )
        submenuButtonSeed.bind(
            "<Leave>", lambda e: self.after(500, lambda: submenu._left(self))
        )
    ...
    def _show(self, *args, **kwargs) -> None:
        ...
        if isinstance(self.menu_seed_object, _CDMSubmenuButton):
            self.place(
                ...
                x=(
                    self.menu_seed_object.winfo_x()
                    + self.menu_seed_object.winfo_width()
                )
                / dpi
                + self.padx
                + 1,
            ...
            self.place(
                ...
                y=(
                    self.menu_seed_object.winfo_y()
                    + self.menu_seed_object.winfo_height()
                )
                / dpi
                + self.pady,
            )
    ...
    def _collapseSiblingSubmenus(
        self, button: _CDMOptionButton | _CDMSubmenuButton, *args, **kwargs
    ) -> None:
        for option in self._options_list:
    ...
    def toggleShow(self, *args, **kwargs) -> None:
        ...
        if not self.winfo_manager():
            if widget_base.startswith("!ctktitlemenu") or widget_base.startswith(
                "!ctkmenubar"
            ):
    ...
    def _checkIfMouseLeft(self, event: tk.Event = None) -> None:
        ...
        if not self._get_coordinates(event.x_root, event.y_root):
            ...
            ) and not self.menu_seed_object.parent_menu._get_coordinates(
                event.x_root, event.y_root
            ):
                subMenus = self._getSubMenus()
                if subMenus == [] or all(
                    (not submenu._get_coordinates(event.x_root, event.y_root))
