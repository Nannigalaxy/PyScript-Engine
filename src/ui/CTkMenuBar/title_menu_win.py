...
class CTkTitleMenu(customtkinter.CTkToplevel):
    ...
    def __init__(
        ...
        if not sys.platform.startswith("win"):
            raise OSError(
                "This title menu works only in windows platform, not supported on your system! \nTry the CTkMenuBar instead..."
            )
        ...
        self.master.bind(
            "<Destroy>",
            lambda _: super().destroy() if not self.master.winfo_viewable() else None,
        )
        self.num = 0
    ...
    def add_cascade(self, text=None, **kwargs):
        ...
        self.menu_button.grid(
            row=0, column=self.num, padx=(0, self.padding)
        )
        self.num += 1
